...
class ProblemServerHandler(BaseHTTPRequestHandler):
    def do_OPTIONS(self):
        # This is the "handshake" Chrome performs
    ...
    def do_GET(self):
        ...
        elif self.path == '/get-problems':
            # Return all problems to the extension
            self.send_response(200)
            self.send_header('Access-Control-Allow-Origin', '*')
            self.send_header('Content-Type', 'application/json')
            self.end_headers()
    ...
    def do_POST(self):
        if self.path == '/add-problem':
            content_length = int(self.headers['Content-Length'])
            post_data = self.rfile.read(content_length)

            try:
                new_problems_from_browser = json.loads(post_data.decode('utf-8'))
                existing_problems = load_problems()
                existing_map = {p['link']: p for p in existing_problems}
                ...
                for p in new_problems_from_browser:
                    problem_tags = p.get('tags', [])
                    diff = int(p.get('difficulty', 0))
                    cf_val = dmoj_to_cf(diff) if diff > 0 else 0
                    ...
                    formatted_problem = {
                        ...
                        "link": p.get('link'),
                        "difficulty": diff,
                        "status": p.get('status', 'unsolved'),
                        "type": problem_tags,
                        "cf_rating": cf_val
                    }

                    if p['link'] in existing_map:
                        # Problem exists - merge with status priority
                        existing = existing_map[p['link']]
                        existing_priority = status_priority.get(existing['status'], 0)
                        new_priority = status_priority.get(formatted_problem['status'], 0)

                        # Use the status with higher priority
                        if new_priority > existing_priority:
                            existing['status'] = formatted_problem['status']
                            updated_count += 1

                        # Update other fields (tags, difficulty might have changed)
                        # Merge tags: combine existing tags with new tags (remove duplicates)
                        existing_tags = set(existing.get('type', []))
                        new_tags = set(formatted_problem['type'])
                        existing['type'] = list(existing_tags | new_tags)
                        existing['difficulty'] = formatted_problem['difficulty']
                        existing['cf_rating'] = formatted_problem['cf_rating']
                    else:
                        # New problem
                        existing_problems.append(formatted_problem)
                        existing_map[p['link']] = formatted_problem
                        added_count += 1

                save_problems(existing_problems)
                ...
                response = {
                    "status": "success",
                    "message": f"Added {added_count} new problems, updated {updated_count} existing",
...
def start_server():
    ...
    server = HTTPServer(('localhost', SERVER_PORT), ProblemServerHandler)
...
def get_chrome_extension_storage_path():
    """Get the path to Chrome's extension storage based on OS"""
    system = platform.system()
    extension_id = "YOUR_EXTENSION_ID"  # You'll need to replace this

    if system == "Windows":
        base_path = os.path.join(os.environ['LOCALAPPDATA'], 'Google', 'Chrome', 'User Data', 'Default',
                                 'Local Extension Settings')
    elif system == "Darwin":  # macOS
        base_path = os.path.expanduser('~/Library/Application Support/Google/Chrome/Default/Local Extension Settings')
    elif system == "Linux":
        base_path = os.path.expanduser('~/.config/google-chrome/Default/Local Extension Settings')
    else:
        return None

    return os.path.join(base_path, extension_id)
...
def run_server():
    server_address = ('', 8765) # SERVER_PORT
    # Replace 'YourHandlerClass' with the actual class name in your script
    httpd = HTTPServer(server_address, ProblemServerHandler)
    print("Server started on port 8765...")
    httpd.serve_forever()


if __name__ == "__main__":
    # 1. Start the server (which uses ProblemServerHandler) in a background thread
    # This prevents the server from freezing your GUI window
    print(f"Starting background server on port {SERVER_PORT}...")
    start_server()

    # 2. Start the PyQt6 GUI
